...
Private Sub Form_Load()
    ...
    sDefinition = "ws := [REPEAT ' ' MIN 0];" & _
                        ...
                        "range := AND ?level0, ['..'], ?level0;" & _
                        "bit_range := LIST (OR range, level0), [';'];" & _
    ...
    sDefinition = sDefinition & _
                        "level0 := OR (AND ['{'], (LIST level0, [AND ws, ';', ws]), ['}']), level1;" & _
                        "level1 := OR keyword, assignment, definition, level2;" & _
                        "level2 := LIST level3, (AND ws, (OR '<>', '>=', '<=', '==', '<', '>'), ws);" & _
                        "level3 := LIST level4, (AND ws, (OR CASE 'and', CASE 'or', CASE 'xor', CASE 'mod'), ws);" & _
                        "level4 := LIST level5, (AND ws, (OR '+','-','&'), ws);" & _
                        "level5 := LIST level6, (AND ws, (OR '*','/','\'), ws);" & _
                        "level6 := LIST level7, (AND ws, '^', ws);" & _
                        "level7 := OR loop, factorial, percent, time, radixnumber, number, function_bit_expression, functionexpressionX, constant, function_variable_call, unaryexpression, bracketexpression;" & _
                        "level7dummy := OR FAIL, FAIL, FAIL, FAIL, FAIL, number, FAIL, FAIL, FAIL, function_variable_call, FAIL, bracketexpression;" & _
                        "factorial := AND level7dummy, ['!'];" & _
                        "percent := AND number, ['%'];" & _
                        "functionexpressionX := AND function, ws, ['('], ws, (LIST level0, [AND ws, ',', ws]), [')'], ws;" & _
                        "function_bit_expression := AND function2, ws, ['('], ws, level0, ws, [','], ws, bit_range, ws, [')'], ws;" & _
                        ...
                        "keyword:= OR CASE 'list', CASE 'help';"
        ...
        Case 1 ' Keyword
            Select Case LCase$(oExpression.Text)
                Case "list"
                    ListDefinitions
                Case "help"
                    InsertText vbCrLf & "SIN, COS, TAN, EXP, LOG, DEG, RAD, INT, FRAC, SQR, NOT, ATN, ACS, ASN, GAM, COT, CSC, SEC, DMS, LEN, RADIX, LOG, FIX, IF, AND, OR, XOR, MOD, LOOP, SHOW" & vbCrLf & "function(x):=func(x)" & vbCrLf & "loop/show+*(a,b) x:x"
