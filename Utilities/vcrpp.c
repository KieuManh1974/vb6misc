...
static numname chan_to_name[] = {	/* Trimmed for anonymity */
    {02, "WAAB2"},
    {03, "WAAC3"},
    {04, "WAAD4"},
    {05, "WAAE5"},
...
void clear_ndigits (unsigned char *a) {
    int             i;
    for (i = 0; i < NDIGITS; i++) {
		a[i] = 0;
    }
...
static int encode_final_transform (x, y)
int             x, y;
{
    int             i, j, digit, sum;
    int             a[9], b[9], out[17];
...
void twiddle_tt (int tidx, int *tp, int *dp, int *x1p, int *x2p) {
    int             tt, x1, x2, t, d;
    int             t1, t2, b;
...
static void bit_shuffle (code, tval, dval, cval)
	int             code;
	unsigned int   *tval;
	unsigned int   *dval;
	unsigned int   *cval;
...
static void interleave (tval, cval, top5out, bot3out)
	int             tval, cval;
	int            *top5out;
	int            *bot3out;
{
...
int revluk (int starttimem, int durationm)
{				/* Both in minutes. */
    int             t, d, x1, x2;
    int             tidx, startm, dur;
...
void map_top (year, month_out, day_out, top5, rem, mtoutp, remoutp)
	int             year;
	int             month_out;
	int             day_out;
	int             top5;
...
void decode_main (int month_today, int day_today, int year_today, int newspaper, int *day_ret, int *channel_ret, int *starttime_ret, int *duration_ret) {
...
int encode_main (int month, int day, int year, int channel, int starttimem, int durationm)
{
    int             j = 0;
    int             s1_out = 0;
    int             bot3 = 0;
...
void main_debug ()
{
	...
	while (tp->code > 0) {
	    ...
	    decode_main (g_month_today, g_day_today, g_year_today, g_newspaper, &day_out, &channel_out, &starttime_out, &duration_out);

	    starttime_out = mmmm2hhmm (starttime_out);

	    if ((day_out == tp->day) && (channel_out == tp->chan) && (starttime_out == tp->start) && (duration_out == tp->duration)) {
			if (g_verbose) {
				printf ("Code: %d %d-%d should be %d, %d, %d, %d\n", g_newspaper, tp->year, tp->month, tp->day, tp->chan, tp->start, tp->duration);
	    ...
	    g_newspaper = encode_main (tp->month, tp->day, tp->year, tp->chan, hhmm2mmmm (tp->start), tp->duration);
	    if (g_verbose) {
	...
	char           *argv[];
{
    ...
    if (0 == g_encode) {
		...
		if (g_verbose) {
			printf ("Today's date: %s %d %d\n", monthname[g_month_today - 1], g_day_today, g_year_today);
		}
		...
		printf ("Program: %s %d %d, channel %d (%s), at %s, duration %d:%2.2d.\n", monthname[g_month_today - 1], day_out, g_year_today, channel_out, g_channel_name, timestr (mmmm2hhmm (starttime_out)), duration_out / 60, duration_out % 60);
    } else {
		/* Encoding */
		g_newspaper = encode_main (g_month_today, g_day_today, g_year_today, g_channel, g_starttimem, g_durationm);

		if (g_verbose) {	/* Announce what we're decoding */
			printf ("%2d %2d %2d %3d %4d %3d  VCR++ code=%8d\n", g_month_today, g_day_today, g_year_today, g_channel, g_starttime, g_duration, g_newspaper);
		} else {
			printf ("%d\n", g_newspaper);
		}
		exit (0);
