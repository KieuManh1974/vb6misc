<?php
	...
	class Parser {
		...
		private function CompileRule($tree) {
			$rule_name = $tree->raw_text($this->stream,0);
			$this->CompileExpression($tree->path(1), $rule_name);
			$this->CompileExpression($tree->path(1), $rule_name.'_',false,false);
			$this->CompileExpression($tree->path(1), $rule_name.'_o',false,true);
			$this->CompileExpression($tree->path(1), $rule_name.'_n',true.false);
			$this->CompileExpression($tree->path(1), $rule_name.'_on',true,true);
		}
		
		private function CompileExpression($tree, $rule_name='',$omit_override = null, $non_override = null) {
			$omit = $omit_override===null?($tree->path(0,0)!==false?($tree->index(0,0)==1):false):$omit_override;
			$non = $non_override===null?($tree->path(0,1)!==false?($tree->index(0,1)==1):false):$non_override;
			...
			switch ($expression_type) {
				...
				case 4: // set
					...
					foreach ($tree->path(1,0,1,1,0)->nodes as $char) {
						switch ($char->index) {
							case 1:
								$literal_set .= '|';
							case 2:
				...
				case 5: // not
				case 6: // opt
					$condition = &$this->CompileExpression($tree->path(1,0));
					$object->def($condition);
					break;
				case 7: // and
				case 8:	// or	
					$object->set = array();
					foreach ($tree->path(1,0,1)->nodes as $node) {
						$object->set[] = &$this->CompileExpression($node);
					}
					$object->length = count($object->set);
					break;
				case 9: // list
					$object->condition = &$this->CompileExpression($tree->path(1,0,1));
					if ($tree->index(1,0,2)==1) {
						$object->delimiter = &$this->CompileExpression($tree->path(1,0,2,0,1));
					} else {
						$object->delimiter = null;
					}

					if ($tree->index(1,0,3)==1) {
						$object->terminator = &$this->CompileExpression($tree->path(1,0,3,0,1));
					} else {
