...
Private Sub mnuCopy_Click()
    ...
    If mlSelection <> -1 Then
        Clipboard.Clear
        Clipboard.SetText moNotes.Displaylist.List(mlSelection + 1).Text, vbCFText
    Else
        For lIndex = 1 To moNotes.Tree.Top.Children.Count
            sText = sText & vbCrLf & moNotes.Tree.Top.Children(lIndex).Text
        Next
        Clipboard.Clear
...
Private Sub mnuPaste_Click()
    If mlSelection <> -1 Then
        moNotes.AddTaskTreeFromText moNotes.Displaylist.List(mlSelection + 1), Clipboard.GetText(vbCFText)
        ShowNotes mlSelection
    Else
        moNotes.AddTaskTreeFromText moNotes.Tree.Top, Clipboard.GetText(vbCFText)
        ShowNotes
    End If
End Sub
